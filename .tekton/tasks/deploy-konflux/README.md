...
- `overrides-yaml` (default: empty): optional inline YAML for `apply-overrides-from-yaml.sh`.
...
2. **deploy-prep** — same task-runner image: optional overrides (`kubectl kustomize` shim for `kustomize build`), then `deploy-local.sh` with `OPERATOR_INSTALL_METHOD=none` (needs credentials below).
