...
and named `build-pipline-<component-name>`.
...
2. :gear: Add the secret to the component's `build-pipline-<component-name>` service account:

```bash
kubectl patch -n $NS serviceaccount "build-pipline-${COMPONENT_NAME}" -p '{"secrets": [{"name": "regcred"}]}'
```
