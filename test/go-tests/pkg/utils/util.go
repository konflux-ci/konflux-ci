...
import (
	...
	"github.com/konflux-ci/konflux-ci/test/go-tests/pkg/rulesengine"
...
func GetChangedFiles(repo string) (rulesengine.Files, error) {
	...
	// else it will run locally which is from the repo directory
	args := []string{"diff", "--name-status", "upstream/main..HEAD"}
	if repo == "infra-deployments" {
		wd, _ := os.Getwd()
		args = append([]string{"-C", fmt.Sprintf("%s/tmp/%s", wd, repo)}, args...)
	}
	cmd := exec.Command("git", args...)
	output, err := cmd.Output()
	if err != nil {
		klog.Error("Failed to run git diff locally")
		return nil, err
	}
	...
	for _, line := range strings.Split(strings.TrimSuffix(string(output), "\n"), "\n") {
		...
		fileAttr := strings.SplitN(line, "\t", 2)
		if len(fileAttr) < 2 {
			continue
		}
