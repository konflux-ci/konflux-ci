...
# Determine how to provide the private key (optional: support both formats)
if [ -n "${GITHUB_PRIVATE_KEY_PATH:-}" ] && [ -f "${GITHUB_PRIVATE_KEY_PATH}" ]; then
    # Use file path if provided
    PRIVATE_KEY_ARG="--from-file=github-private-key=${GITHUB_PRIVATE_KEY_PATH}"
else
    # Use literal key content (validated earlier with :?)
    PRIVATE_KEY_ARG="--from-literal=github-private-key=${GITHUB_PRIVATE_KEY}"
fi
