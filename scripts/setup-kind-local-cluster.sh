...
#
# Prerequisites:
# - kind: brew install kind (or appropriate package manager)
# - podman: brew install podman (macOS) or docker (Linux)
# - kubectl: brew install kubectl
#
# Configuration:
# Set these variables in your deploy-local-dev.env file:
...
if [[ "$(uname)" == "Darwin" ]] && command -v podman &> /dev/null; then
    ...
    if [ -n "${PODMAN_MEMORY}" ] && [ "${PODMAN_MEMORY}" -lt "${REQUIRED_MEMORY_MB}" ]; then
        ...
        echo ""
        echo "Or reduce KIND_MEMORY_GB in scripts/deploy-local-dev.env"
        exit 1
    fi
...
if [[ "${ENABLE_REGISTRY_PORT}" -eq 1 ]]; then
    ...
    if command -v lsof &> /dev/null; then
        if lsof -i ":${REGISTRY_HOST_PORT}" >/dev/null 2>&1; then
            ...
            echo "  2. Choose a different port by setting REGISTRY_HOST_PORT in scripts/deploy-local-dev.env"
            echo "  3. Disable registry port binding by setting ENABLE_REGISTRY_PORT=0 in scripts/deploy-local-dev.env"
            echo ""
...
if [[ "$HOST_ARCH" == "arm64" ]] || [[ "$HOST_ARCH" == "aarch64" ]]; then
    echo "Detected ARM64 architecture - using kind-config-arm64.yaml..."
    KIND_CONFIG="${REPO_ROOT}/kind-config-arm64.yaml"
else
    echo "Detected ${HOST_ARCH} architecture - using kind-config.yaml..."
...
echo ""
echo "Or use the all-in-one script: ./scripts/deploy-local-dev.sh my-konflux.yaml"
