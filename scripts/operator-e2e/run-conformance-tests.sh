...
if [[ -n "${E2E_CONFORMANCE_IMAGE:-}" ]]; then
  ...
  # Build JSON array tail for any extra args; shell word-splitting is intentional here.
  # shellcheck disable=SC2086
  EXTRA_ARGS_JSON=""
  for _arg in ${E2E_CONFORMANCE_GO_TEST_EXTRA_ARGS:-}; do
    EXTRA_ARGS_JSON+=", \"${_arg}\""
  done

  # --overrides replaces the containers array; all fields including args must be
  # declared here — args passed after -- are discarded.
    ...
    --overrides="$(cat <<OVERRIDES
{
  "spec": {
    "containers": [{
      "name": "${POD_NAME}",
      "image": "${E2E_CONFORMANCE_IMAGE}",
      "args": ["-ginkgo.vv", "-test.timeout", "45m", "-test.count", "1"${EXTRA_ARGS_JSON}],
      "env": [
