...
Every run (pass or fail) emits `[UWM scrape]` evidence lines after UWM readiness for scrape-token component targets (secret/SM resource versions, `sm_after_secret`, `uwm_active_targets` / `uwm_up`). After scrape-token mint/refresh, operand reconcilers write `metrics-scrape-resync*` annotations (`token-minted` / `token-refreshed`, then `settle-retry` after the settle deadline); metrics-contract specs require those annotations to be **present**. Prefer `sm_after_secret=true` and `uwm_active_targets=1` / `uwm_up=1` as pass fingerprints.
...
| `run-conformance-tests.sh` | Runs conformance tests. Two modes: **source** (default, `go test`) and **image** (`E2E_CONFORMANCE_IMAGE` set — runs the pre-built conformance container as a pod with kubeconfig and credentials injected). Requires `GH_ORG`, `GH_TOKEN` (conformance clears `QUAY_TOKEN` for the test run). |
