...
const (
	...
	// FieldManagerInternalRegistry is the field manager name for internal registry resources
	FieldManagerInternalRegistry = "konflux-internal-registry-controller"
)
...
func (r *KonfluxInternalRegistryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	tc := tracking.NewClientWithOwnership(r.Client, tracking.OwnershipConfig{
		...
		FieldManager:      FieldManagerInternalRegistry,
	})

	// Apply manifests (if CR exists, it's enabled)
	if err := r.applyManifests(ctx, tc); err != nil {
		log.Error(err, "Failed to apply manifests")
		SetFailedCondition(registry, InternalRegistryConditionTypeReady, "ApplyFailed", err)
	...
	if err := UpdateComponentStatuses(ctx, r.Client, registry, InternalRegistryConditionTypeReady); err != nil {
		log.Error(err, "Failed to update component statuses")
		SetFailedCondition(registry, InternalRegistryConditionTypeReady, "FailedToGetDeploymentStatus", err)
		if updateErr := r.Status().Update(ctx, registry); updateErr != nil {
			log.Error(updateErr, "Failed to update status")
		}
		return ctrl.Result{}, err
...
func (r *KonfluxInternalRegistryReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		...
		Owns(&corev1.Service{}, builder.WithPredicates(labelsOrAnnotationsChangedPredicate)).
		Owns(&corev1.Namespace{}, builder.WithPredicates(labelsOrAnnotationsChangedPredicate)).
		Complete(r)
}
