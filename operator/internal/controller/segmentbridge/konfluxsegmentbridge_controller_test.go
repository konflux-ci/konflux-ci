...
var _ = Describe("KonfluxSegmentBridge Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("ensuring the konflux-info namespace exists")
			ns := &corev1.Namespace{}
			err := k8sClient.Get(ctx, types.NamespacedName{Name: uiNamespace}, ns)
			if err != nil && errors.IsNotFound(err) {
				ns = &corev1.Namespace{
					ObjectMeta: metav1.ObjectMeta{Name: uiNamespace},
				}
