...
func (r *KonfluxSegmentBridgeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if segmentKey != "" {
		if err := r.reconcileSegmentBridgeSecret(ctx, tc, segmentKey); err != nil {
...
func (r *KonfluxSegmentBridgeReconciler) reconcileSegmentBridgeSecret(ctx context.Context, tc *tracking.Client, segmentKey string) error {
	log := logf.FromContext(ctx)
