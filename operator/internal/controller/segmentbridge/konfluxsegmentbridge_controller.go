...
// manifestSource is the narrow slice of *manifests.ObjectStore's API that this
// reconciler needs. Depending on this interface (rather than the concrete type)
// lets tests inject manifest-loading and object-shape failures directly, so the
// error-handling branches in applyManifests are exercised for real instead of
// being structurally unreachable through the always-succeeding embedded manifests.
type manifestSource interface {
	GetForComponent(component manifests.Component) ([]client.Object, error)
}
