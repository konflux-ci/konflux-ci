...
var _ = Describe("KonfluxDefaultTenant Controller", func() {
	Context("When reconciling a resource", Ordered, func() {
		BeforeAll(func() {
			...
			// Wait once for the controller to finish its first reconcile.
			// Once Ready=True all owned resources are guaranteed to exist.
		...
		AfterAll(func() {
			...
			ns := &corev1.Namespace{}
			if err := k8sClient.Get(ctx, types.NamespacedName{Name: defaultTenantNamespace}, ns); err == nil {
