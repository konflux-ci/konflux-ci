...
func (r *KonfluxDefaultTenantReconciler) syncInternalRegistryCredentialsSecret(
	...
	if err := r.ensureIntegrationRunnerRegistryCreds(ctx); err != nil {
		return ctrl.Result{}, err
	}
...
// ensureIntegrationRunnerRegistryCreds patches the konflux-integration-runner ServiceAccount in
// default-tenant so imagePullSecrets and secrets list RegistryCredentialsSecretName. That lets
// integration and Conforma PipelineRuns authenticate to the internal registry after the copied
// dockerconfig secret exists. It is a no-op when the references are already present. Returns an
// error if the ServiceAccount is missing (bootstrap ordering) or the patch fails.
func (r *KonfluxDefaultTenantReconciler) ensureIntegrationRunnerRegistryCreds(ctx context.Context) error {
	sa := &corev1.ServiceAccount{}
	key := types.NamespacedName{Namespace: DefaultTenantNamespace, Name: IntegrationRunnerServiceAccountName}
	if err := r.Get(ctx, key, sa); err != nil {
		if apierrors.IsNotFound(err) {
			return fmt.Errorf("serviceaccount %s/%s not found yet", DefaultTenantNamespace, IntegrationRunnerServiceAccountName)
		}
	...
	if !mergeRegistryCredRefsIntoServiceAccount(sa, RegistryCredentialsSecretName) {
		return nil
	}
	if err := r.Patch(ctx, sa, client.MergeFrom(original)); err != nil {
		return fmt.Errorf("patch serviceaccount %s/%s for registry credentials: %w", DefaultTenantNamespace, IntegrationRunnerServiceAccountName, err)
	}
	return nil
}
...
// is gone, so stale pull credentials are not left on the cluster. Missing ServiceAccount is
// ignored. Returns an error only on unexpected get/patch failures.
func (r *KonfluxDefaultTenantReconciler) ensureIntegrationRunnerRegistryCredsRemoved(ctx context.Context) error {
	...
	original := sa.DeepCopy()
	if !stripRegistryCredRefsFromServiceAccount(sa, RegistryCredentialsSecretName) {
		return nil
	}
	if err := r.Patch(ctx, sa, client.MergeFrom(original)); err != nil {
...
// mergeRegistryCredRefsIntoServiceAccount mutates sa by appending secretName to imagePullSecrets
// and secrets when not already listed. Returns true if either slice was modified.
func mergeRegistryCredRefsIntoServiceAccount(sa *corev1.ServiceAccount, secretName string) bool {
	changed := false
...
// stripRegistryCredRefsFromServiceAccount mutates sa by removing secretName from imagePullSecrets
// and secrets. Returns true if either slice changed.
func stripRegistryCredRefsFromServiceAccount(sa *corev1.ServiceAccount, secretName string) bool {
	changed := false
...
// localObjectRefContainsName reports whether refs contains a LocalObjectReference with the given name.
...
// objectRefContainsName reports whether refs contains an ObjectReference with the given name.
