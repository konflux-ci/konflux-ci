...
type buildServiceFakeTokenCreator struct {
	token     string
	expiresAt time.Time
	calls     int
}

func (f *buildServiceFakeTokenCreator) CreateScraperToken(_ context.Context, _ types.NamespacedName, _ time.Duration) (string, time.Time, error) {
	f.calls++
...
var _ = Describe("Prometheus scrape token", func() {
	...
	Context("when component metrics are enabled", func() {
		It("creates prometheus-scrape-token and wires ServiceMonitor bearerTokenSecret", func(ctx context.Context) {
			...
			fake := &buildServiceFakeTokenCreator{
