...
func TestApplyPipelineConfigMerge(t *testing.T) {
	...
	t.Run("removeDefaults with no pipelines returns error for dangling default", func(t *testing.T) {
		g := gomega.NewWithT(t)
		cm := makeConfigMap()
		err := applyPipelineConfigMerge(cm, &konfluxv1alpha1.PipelineConfigSpec{
			RemoveDefaults: true,
		...
		g.Expect(err.Error()).To(gomega.ContainSubstring("docker-build-oci-ta"))
	...
	t.Run("error when current default is removed without setting new defaultPipelineName", func(t *testing.T) {
		g := gomega.NewWithT(t)
		cm := makeConfigMap()
		err := applyPipelineConfigMerge(cm, &konfluxv1alpha1.PipelineConfigSpec{
			Pipelines: []konfluxv1alpha1.PipelineSpec{
