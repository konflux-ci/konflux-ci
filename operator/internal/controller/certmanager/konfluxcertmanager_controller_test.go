...
var _ = Describe("KonfluxCertManager Controller", Ordered, func() {
	...
	Context("When reconciling a resource", func() {
		...
		clusterIssuerNames := testutil.ResourceNamesFromManifest(manifests.CertManager, "ClusterIssuer")
		certificateNames := testutil.ResourceNamesFromManifest(manifests.CertManager, "Certificate")

		// cleanupIssuers removes all ClusterIssuers and Certificates the controller may have created.
		cleanupIssuers := func() {
			for _, name := range clusterIssuerNames {
				obj := &unstructured.Unstructured{}
				obj.SetGroupVersionKind(schema.GroupVersionKind{Group: "cert-manager.io", Version: "v1", Kind: "ClusterIssuer"})
				obj.SetName(name)
				testutil.DeleteAndWait(ctx, k8sClient, obj)
			}
			for _, name := range certificateNames {
				...
				cert.SetName(name)
				cert.SetNamespace(certManagerNamespace)
				testutil.DeleteAndWait(ctx, k8sClient, cert)
			}
		}

		clusterIssuer := func(name string) *unstructured.Unstructured {
			obj := &unstructured.Unstructured{}
			obj.SetGroupVersionKind(schema.GroupVersionKind{Group: "cert-manager.io", Version: "v1", Kind: "ClusterIssuer"})
			obj.SetName(name)
			return obj
		...
		certificate := func(name, namespace string) *unstructured.Unstructured {
			obj := &unstructured.Unstructured{}
		...
		Context("with createClusterIssuer unset (defaults to enabled)", Ordered, func() {
			BeforeAll(func() {
			...
			It("should create the ClusterIssuers", func() {
				for _, name := range clusterIssuerNames {
					Expect(k8sClient.Get(ctx, types.NamespacedName{Name: name}, clusterIssuer(name))).
						To(Succeed(), "expected ClusterIssuer %q to exist", name)
				}
			})

			It("should create the Certificates", func() {
				for _, name := range certificateNames {
					Expect(k8sClient.Get(ctx, types.NamespacedName{Name: name, Namespace: certManagerNamespace}, certificate(name, certManagerNamespace))).
						To(Succeed(), "expected Certificate %q to exist in namespace %q", name, certManagerNamespace)
				}
			})
		})

		Context("with createClusterIssuer explicitly enabled", Ordered, func() {
			BeforeAll(func() {
				enabled := true
				Expect(k8sClient.Create(ctx, &konfluxv1alpha1.KonfluxCertManager{
					ObjectMeta: metav1.ObjectMeta{Name: CRName},
		...
		Context("with createClusterIssuer disabled", Ordered, func() {
			BeforeAll(func() {
				disabled := false
				Expect(k8sClient.Create(ctx, &konfluxv1alpha1.KonfluxCertManager{
					ObjectMeta: metav1.ObjectMeta{Name: CRName},
			...
			It("should NOT create any ClusterIssuers", func() {
				for _, name := range clusterIssuerNames {
					err := k8sClient.Get(ctx, types.NamespacedName{Name: name}, clusterIssuer(name))
					Expect(errors.IsNotFound(err)).To(BeTrue(), "ClusterIssuer %q should not exist", name)
				}
			})

			It("should NOT create any Certificates", func() {
				for _, name := range certificateNames {
					err := k8sClient.Get(ctx, types.NamespacedName{Name: name, Namespace: certManagerNamespace}, certificate(name, certManagerNamespace))
					Expect(errors.IsNotFound(err)).To(BeTrue(), "Certificate %q should not exist", name)
				}
			})
		})
	})
