...
type imageControllerFakeTokenCreator struct {
	token     string
	expiresAt time.Time
	calls     int
}

func (f *imageControllerFakeTokenCreator) CreateScraperToken(
	_ context.Context,
...
func reconcileImageController(ctx context.Context, reconciler *KonfluxImageControllerReconciler) {
...
func ensureComponentMetricsDisabled(ctx context.Context) {
	disabled := false
	konflux := &konfluxv1alpha1.Konflux{}
	err := k8sClient.Get(ctx, types.NamespacedName{Name: constant.KonfluxSingletonName}, konflux)
	if apierrors.IsNotFound(err) {
		Expect(k8sClient.Create(ctx, &konfluxv1alpha1.Konflux{
	...
	konflux.Spec.ComponentMetrics = &konfluxv1alpha1.ComponentMetricsConfig{Enabled: &disabled}
	Expect(k8sClient.Update(ctx, konflux)).To(Succeed())
}

var _ = Describe("Prometheus scrape token", Serial, func() {
	...
	Context("when component metrics are enabled", func() {
		It("creates prometheus-scrape-token and wires ServiceMonitor bearerTokenSecret", func(ctx context.Context) {
			...
			fake := &imageControllerFakeTokenCreator{
		...
		It("refreshes the scrape token after the rotation threshold", func(ctx context.Context) {
			...
			reconcileImageController(ctx, reconciler)
	...
	Context("when component metrics are disabled", func() {
		It("skips ServiceMonitor apply", func(ctx context.Context) {
			ensureComponentMetricsDisabled(ctx)
			DeferCleanup(func(ctx context.Context) {
				_ = client.IgnoreNotFound(k8sClient.Delete(ctx, &konfluxv1alpha1.Konflux{
					ObjectMeta: metav1.ObjectMeta{Name: constant.KonfluxSingletonName},
				}))
			})

			mgr := testutil.NewTestManager(testEnv)
			fake := &imageControllerFakeTokenCreator{token: "unused", expiresAt: time.Now().Add(time.Hour)}
			reconciler := &KonfluxImageControllerReconciler{
				Client:       k8sClient,
				Scheme:       mgr.GetScheme(),
				ObjectStore:  objectStore,
