...
const (
	// CRName is the singleton name for the KonfluxCLI CR.
	CRName = "konflux-cli"
	// FieldManager is the field manager identifier for server-side apply.
	FieldManager = "konflux-cli-controller"
	// crKind is used in error messages to identify this CR type.
	crKind = "KonfluxCLI"
)

// CLICleanupGVKs defines which resource types should be cleaned up when they are
...
var CLICleanupGVKs = []schema.GroupVersionKind{}

// KonfluxCLIReconciler reconciles a KonfluxCLI object
type KonfluxCLIReconciler struct {
	client.Client
	Scheme      *runtime.Scheme
	ObjectStore *manifests.ObjectStore
}

// +kubebuilder:rbac:groups=konflux.konflux-ci.dev,resources=konfluxclis,verbs=get;list;watch;create;update;patch;delete
...
func (r *KonfluxCLIReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := logf.FromContext(ctx)

	konfluxCLI := &konfluxv1alpha1.KonfluxCLI{}
	if err := r.Get(ctx, req.NamespacedName, konfluxCLI); err != nil {
		return ctrl.Result{}, client.IgnoreNotFound(err)
	}

	log.Info("Reconciling KonfluxCLI", "name", konfluxCLI.Name)

	errHandler := condition.NewReconcileErrorHandler(log, r.Status(), konfluxCLI, crKind)
	...
	tc := tracking.NewClientWithOwnership(r.Client, tracking.OwnershipConfig{
		...
		Component:         string(manifests.CLI),
	...
	if err := tc.CleanupOrphans(ctx, constant.KonfluxOwnerLabel, konfluxCLI.Name, CLICleanupGVKs); err != nil {
		return errHandler.HandleCleanupError(ctx, err)
	}

	if err := condition.UpdateComponentStatuses(ctx, r.Client, konfluxCLI); err != nil {
		return errHandler.HandleStatusUpdateError(ctx, err)
	}

	if err := r.Status().Update(ctx, konfluxCLI); err != nil {
		log.Error(err, "Failed to update status")
		return ctrl.Result{}, err
	}

	log.Info("Successfully reconciled KonfluxCLI")
	return ctrl.Result{}, nil
}

func (r *KonfluxCLIReconciler) applyManifests(ctx context.Context, tc *tracking.Client) error {
	objects, err := r.ObjectStore.GetForComponent(manifests.CLI)
	if err != nil {
		return fmt.Errorf("failed to get manifests for CLI: %w", err)
	}

	for _, obj := range objects {
		if err := tc.ApplyOwned(ctx, obj); err != nil {
			return fmt.Errorf("failed to apply object %s/%s (%s) from %s: %w",
				obj.GetNamespace(), obj.GetName(), tracking.GetKind(obj), manifests.CLI, err)
		}
	}
	return nil
}
...
func (r *KonfluxCLIReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&konfluxv1alpha1.KonfluxCLI{}).
		Named("konfluxcli").
