...
# Creates Application, Component, and optionally a Pipelines-as-Code Repository; optionally
# an IntegrationTestScenario from custom git (-i/-j/-k); optionally a UI-equivalent Conforma
# IntegrationTestScenario (--include-conforma-scenario).
...
Options:
  ...
  -k, --integration-path        Integration test pathInRepo (optional; requires -i and -j)
      --include-conforma-scenario  Add UI-equivalent Conforma IntegrationTestScenario
                                   (build-definitions @ main, enterprise-contract pipeline)
      --skip-conforma-scenario   Same as default (no Conforma scenario); kept for compatibility
...
INTEGRATION_PATH=""
SKIP_CONFORMA_SCENARIO=true
REGISTRY_SECRET_NAME="regcred-internal-registry"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -k|--integration-path)
            ...
            shift 2
            ;;
        --include-conforma-scenario)
            SKIP_CONFORMA_SCENARIO=false
            shift 1
            ;;
        --skip-conforma-scenario)
            SKIP_CONFORMA_SCENARIO=true
            shift 1
            ;;
        -h|--help)
...
if [[ "${has_all_integration_values}" == "true" ]]; then
    ...
    echo "  Integration test:  not configured (git resolver)"
fi
if [[ "${SKIP_CONFORMA_SCENARIO}" == "true" ]]; then
    echo "  Conforma scenario (UI-style): not created (use --include-conforma-scenario to add)"
else
    echo "  Conforma scenario (UI-style): ${APPLICATION}-enterprise-contract @ build-definitions/main"
fi
echo ""
...
if [[ "${SKIP_CONFORMA_SCENARIO}" != "true" ]]; then
    echo "Applying Conforma IntegrationTestScenario (${APPLICATION}-enterprise-contract)..."
    kubectl apply -f - <<EOF
...
if [[ "${REGISTRY_MODE}" == "in-cluster" ]]; then
    ...
    if ! kubectl get secret "${REGISTRY_SECRET_NAME}" -n "${TENANT_NS}" >/dev/null 2>&1; then
        echo "Error: expected secret '${REGISTRY_SECRET_NAME}' in namespace '${TENANT_NS}' for in-cluster registry mode."
        echo "This secret should be provided by the operator (default-tenant) or copied by a cluster administrator."
        exit 1
    fi
    ...
    # imagePullSecrets: kubelet injects these for pulling *container* images in the pod spec.
    # secrets: Konflux/Tekton build steps (e.g. buildah push) still expect dockerconfigjson secrets
    # listed here so creds are materialized under /tekton/creds for in-pod registry auth — imagePullSecrets alone is not enough.
    echo "Attaching secret '${REGISTRY_SECRET_NAME}' to ServiceAccount '${BUILD_PIPELINE_SA}' (secrets + imagePullSecrets)..."
    # Strategic merge merges list entries by name; JSON merge patch (--type merge) would replace entire lists.
    kubectl patch serviceaccount "${BUILD_PIPELINE_SA}" -n "${TENANT_NS}" \
      --type strategic \
      -p "{\"secrets\":[{\"name\":\"${REGISTRY_SECRET_NAME}\"}],\"imagePullSecrets\":[{\"name\":\"${REGISTRY_SECRET_NAME}\"}]}" >/dev/null
fi
