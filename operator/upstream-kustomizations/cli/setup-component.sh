...
# Creates Application, Component, Pipelines-as-Code Repository, a Conforma
# IntegrationTestScenario (same shape as the Konflux UI, build-definitions @ main), and
# an optional additional IntegrationTestScenario from custom git (-i/-j/-k).
...
Options:
  ...
  -k, --integration-path        Integration test pathInRepo (optional; requires -i and -j)
      --skip-conforma-scenario   Skip the UI-equivalent Conforma IntegrationTestScenario
...
if [[ "${SKIP_CONFORMA_SCENARIO}" == "true" ]]; then
    echo "  Conforma scenario (UI-style): skipped (--skip-conforma-scenario)"
else
