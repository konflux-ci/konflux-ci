...
2. Run the script to onboard your fork into the default tenant namespace and use the
   ...
   The script creates a **Conforma** `IntegrationTestScenario` named
   `<application>-enterprise-contract` (same shape as the Konflux UI: git resolver to
   `konflux-ci/build-definitions` at `main`, path `pipelines/enterprise-contract.yaml`).
   Pass `--skip-conforma-scenario` if you do not want that scenario.
