...
2. Run the script to onboard your fork into the default tenant namespace and use the
   ...
   ./setup-component.sh \
     ...
     -i "$REPO" \
     -j main \
     -k integration-tests/testrepo-integration.yaml
   ```

   The script creates Application, Component, a Pipelines-as-Code Repository, and the
   sample git `IntegrationTestScenario` from the flags above. It does **not** create the
   UI-style Conforma `IntegrationTestScenario` unless you add `--include-conforma-scenario`
   (git resolver to `konflux-ci/build-definitions` at `main`, path `pipelines/enterprise-contract.yaml`).
