...
| `setup-component` | `setup-component.sh` | Onboard an application component, a UI-style Conforma `IntegrationTestScenario` (`build-definitions` @ `main`), and optionally a custom git `IntegrationTestScenario` |
...
It creates Application, Component, and Repository resources, a Conforma
`IntegrationTestScenario` (UI-equivalent, `build-definitions` @ `main`), and can optionally
create an additional `IntegrationTestScenario` from custom git (`-i`/`-j`/`-k`).
...
Run `./setup-component.sh -h` for the full list of options (including `--skip-conforma-scenario`).
