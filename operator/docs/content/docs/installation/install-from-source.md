...
```bash
# Generic Kubernetes
SKIP_INTERNAL_REGISTRY=true SKIP_SMEE=true ./deploy-deps.sh

# OpenShift - use native operators instead of upstream ones
USE_OPENSHIFT_PIPELINES=true USE_OPENSHIFT_CERTMANAGER=true \
SKIP_TRUST_MANAGER=true SKIP_INTERNAL_REGISTRY=true SKIP_SMEE=true \
./deploy-deps.sh
```
...
In a **separate terminal**, see
[Applying the Konflux Custom Resource]({{< relref "apply-konflux-cr" >}}) for instructions
