...
type trustedTasksSource struct {
	Kind string
	Ref  string
}
...
func TestOnboardingPipelineTaskBundlesMatchTrustedTasksCatalogHead(t *testing.T) {
	...
	trustedSource, err := extractTrustedTasksSourceRef(ecYAML)
	g.Expect(err).NotTo(gomega.HaveOccurred(),
		"extract trusted_tasks source ref from default EnterpriseContractPolicy")
	if err != nil {
		return
	}

	trusted := fetchTrustedTasksMapEventually(g, trustedSource)
...
func extractTrustedTasksSourceRef(manifest []byte) (trustedTasksSource, error) {
	docs, err := splitYAMLDocuments(manifest)
	if err != nil {
		return trustedTasksSource{}, err
	}
	for _, doc := range docs {
		var u unstructured.Unstructured
		if err := yaml.Unmarshal(doc, &u.Object); err != nil {
		...
		for _, src := range sources {
			...
			for _, row := range data {
				...
				if strings.HasPrefix(s, "oci::") && strings.Contains(s, "data-acceptable-bundles") {
					return trustedTasksSource{
						Kind: "oci",
						Ref:  strings.TrimPrefix(s, "oci::"),
					}, nil
				}

				if strings.Contains(s, "github.com/konflux-ci/konflux-operator-trusted-sources//data?") {
					return trustedTasksSource{
						Kind: "github",
						Ref:  s,
					}, nil
	...
	return trustedTasksSource{}, fmt.Errorf(
		"default EnterpriseContractPolicy trusted_tasks source not found " +
			"(expected data-acceptable-bundles OCI or konflux-operator-trusted-sources git entry)",
	)
}
...
const (
	...
	// Bounds each network call so Gomega Eventually cannot hang on a stuck dial/read.
	registryNetworkTimeout = 90 * time.Second
...
func fetchTrustedTasksMapEventually(g *gomega.WithT, source trustedTasksSource) map[string][]trustedTaskEntry {
	var trusted map[string][]trustedTaskEntry
	g.Eventually(func() error {
		var err error
		trusted, err = fetchTrustedTasksMapOnce(source)
		if err != nil && !isRetriableTrustedTasksSourceError(source, err) {
...
func fetchTrustedTasksMapOnce(source trustedTasksSource) (map[string][]trustedTaskEntry, error) {
	switch source.Kind {
	case "oci":
		return fetchTrustedTasksMapFromOCI(source.Ref)
	case "github":
		return fetchTrustedTasksMapFromGitHub(source.Ref)
	default:
		return nil, fmt.Errorf("unsupported trusted_tasks source kind %q", source.Kind)
	}
}

func fetchTrustedTasksMapFromOCI(imageRef string) (map[string][]trustedTaskEntry, error) {
	ctx, cancel := context.WithTimeout(context.Background(), registryNetworkTimeout)
	defer cancel()
...
func fetchTrustedTasksMapFromGitHub(sourceRef string) (map[string][]trustedTaskEntry, error) {
	rawURL, err := trustedTasksRawGitHubURL(sourceRef)
	if err != nil {
		return nil, err
	}
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("GET %q: unexpected status %d", effectiveURL, resp.StatusCode)
	}
	b, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("read %q: %w", effectiveURL, err)
	}
	var root trustedTasksYAML
	if err := yaml.Unmarshal(b, &root); err != nil {
		return nil, fmt.Errorf("parse trusted_tasks YAML from %q: %w", effectiveURL, err)
	}
	if len(root.TrustedTasks) == 0 {
		return nil, fmt.Errorf("no trusted_tasks entries in %q", effectiveURL)
	}
	return root.TrustedTasks, nil
}

func trustedTasksRawGitHubURL(sourceRef string) (string, error) {
	// Expected source format:
	// github.com/<org>/<repo>//<path>?ref=<ref>
	s := strings.TrimPrefix(sourceRef, "git::")
	if !strings.HasPrefix(s, "github.com/") {
		return "", fmt.Errorf("unsupported git trusted_tasks source %q (expected github.com/...)", sourceRef)
	}
	rest := strings.TrimPrefix(s, "github.com/")
	i := strings.Index(rest, "//")
	if i <= 0 {
		return "", fmt.Errorf("invalid git trusted_tasks source format %q", sourceRef)
	}
	repo := rest[:i]
	pathAndQuery := rest[i+2:]
	pathPart := pathAndQuery
	queryPart := ""
	if qIdx := strings.Index(pathAndQuery, "?"); qIdx >= 0 {
		pathPart = pathAndQuery[:qIdx]
		queryPart = pathAndQuery[qIdx+1:]
	}
	if pathPart == "" {
		return "", fmt.Errorf("missing path in git trusted_tasks source %q", sourceRef)
	}
	// Policy uses a directory URL (//data) for EC; raw.githubusercontent.com needs the file path.
	if pathPart == "data" {
		pathPart = "data/trusted-sources.yaml"
	}
	ref := ""
	if queryPart != "" {
		values, err := url.ParseQuery(queryPart)
		if err != nil {
			return "", fmt.Errorf("parse query in git trusted_tasks source %q: %w", sourceRef, err)
		}
		ref = values.Get("ref")
	}
	if ref == "" {
		return "", fmt.Errorf("missing ?ref= in git trusted_tasks source %q", sourceRef)
	}
	return fmt.Sprintf("https://raw.githubusercontent.com/%s/%s/%s", repo, ref, pathPart), nil
}
...
func isRetriableTrustedTasksSourceError(source trustedTasksSource, err error) bool {
	if source.Kind == "oci" {
		return isRetriableRegistryError(err)
	}
