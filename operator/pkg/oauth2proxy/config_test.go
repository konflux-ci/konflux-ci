...
func TestWithDexCA(t *testing.T) {
	g := NewGomegaWithT(t)

	c := applyOption(WithDexCA())

	// Verify the volume mount is configured correctly
	g.Expect(c.VolumeMounts).To(HaveLen(1))
	mount := c.VolumeMounts[0]
	g.Expect(mount.Name).To(Equal(DexCAVolumeName))
	g.Expect(mount.MountPath).To(Equal(DexCAMountPath))
	g.Expect(mount.SubPath).To(Equal(DexCACertFileName))
