...
func TestPrimaryScraperServiceAccount(t *testing.T) {
	t.Parallel()
	ocp := PrimaryScraperServiceAccount(true)
	if ocp.Namespace != OpenShiftScraperNamespace || ocp.Name != OpenShiftScraperSA {
		t.Fatalf("unexpected openshift scraper: %#v", ocp)
	}
	kind := PrimaryScraperServiceAccount(false)
	if kind.Namespace != KindScraperNamespace || kind.Name != KindScraperSA {
		t.Fatalf("unexpected kind scraper: %#v", kind)
	}
}
...
func TestCreateScraperToken(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	scraper := types.NamespacedName{Namespace: KindScraperNamespace, Name: KindScraperSA}
	now := time.Date(2026, 7, 1, 12, 0, 0, 0, time.UTC)
