...
// can remove resources from the interim infra-deployments model.
func IsComponentMetricsScrapeResource(obj client.Object) bool {
	...
	case rbacv1.SchemeGroupVersion.Group:
		switch gvk.Kind {
		case "ClusterRole":
			return strings.HasSuffix(name, "-metrics-reader")
		case "ClusterRoleBinding":
			return strings.HasPrefix(name, "prometheus-") && strings.HasSuffix(name, "-metrics-reader")
		}
	case "":
		switch gvk.Kind {
		case "ServiceAccount":
...
func isMetricsReaderServiceAccountName(name string) bool {
	return name == "metrics-reader" || strings.HasSuffix(name, "-metrics-reader")
}
