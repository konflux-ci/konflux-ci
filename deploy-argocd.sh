...
login_argocd() {
    argocd login argocd.konflux-ci.net --sso
}
...
deploy_tekton() {
    # Operator
    kubectl apply -k "${ROOT}/dependencies/tekton-operator"
    ...
    retry "kubectl apply -k ${ROOT}/dependencies/tekton-config" \
          "The Tekton Config resource was not updated within the allocated time"

    # Pipeline As Code
    kubectl apply -k "${ROOT}/dependencies/pipelines-as-code"
    ...
    kubectl apply -k "${ROOT}/dependencies/tekton-results"
}

deploy_cert_manager() {
    kubectl apply -k "${ROOT}/dependencies/cert-manager"
    sleep 5
...
deploy_keycloak() {
    kubectl apply -k "${ROOT}/dependencies/keycloak/crd"
    sleep 2 # Give the api time to realize it support new CRDs
    kubectl apply -k "${ROOT}/dependencies/keycloak/deployment"
...
deploy_apps() {
    ...
    deploy_cert_manager
    kubectl apply -k "${ROOT}/dependencies/cluster-issuer"
    deploy_tekton
    deploy_keycloak

    kubectl create -k "${ROOT}/argo-cd-apps/base/member/konflux-ci"
