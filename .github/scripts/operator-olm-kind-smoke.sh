...
#
# Installs the operator via `operator-sdk run bundle` into a namespace that does
# not match the CSV olm.suggested-namespace (mimics preflight's suffixed install
...
#   NS                Install namespace (default: konflux-operator-olm-smoke; must
#                     differ from CSV olm.operatorNamespace if present in bundle)
#                     Set NS=konflux-operator to exercise the suggested namespace.
...
# Default differs from olm.suggested-namespace / olm.operatorNamespace so a bundle
# that incorrectly ships olm.operatorNamespace fails here (preflight uses suffixed NS).
NS="${NS:-konflux-operator-olm-smoke}"
