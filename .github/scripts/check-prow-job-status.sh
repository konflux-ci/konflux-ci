...
for job_name in "${JOBS[@]}"; do
  ...
  # Extract result (guard against malformed JSON)
  RESULT=$(echo "$FINISHED_JSON" | jq -r '.result // empty' 2>/dev/null || echo "")
  PASSED=$(echo "$FINISHED_JSON" | jq -r '.passed // empty' 2>/dev/null || echo "")
  TIMESTAMP=$(echo "$FINISHED_JSON" | jq -r '.timestamp // empty' 2>/dev/null || echo "")

  if [ -z "$RESULT" ] && [ -z "$PASSED" ]; then
    echo "  Warning: Could not parse finished.json"
    continue
  fi
